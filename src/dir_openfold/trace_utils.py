...
import src.dir_openfold.utils_openfold
...
def trace_model_(model, sample_input):
    ...
    m = torch.rand(msa_depth + 4, n, src.dir_openfold.utils_openfold.c_m).to(device)
    z = torch.rand(n, n, src.dir_openfold.utils_openfold.c_z).to(device)
    t = torch.rand(no_templates, n, n, src.dir_openfold.utils_openfold.c_t).to(device)
    a = torch.rand(extra_msa_depth, n, src.dir_openfold.utils_openfold.c_e).to(device)
    ...
    evoformer_attn_chunk_size = max(
        src.dir_openfold.utils_openfold.chunk_size, evoformer_chunk_size // 4
    )
