...
def run_model(model, batch, tag, output_dir, gpu_temp_limit=None, plddt_stop=False):
    with torch.no_grad():
        ...
        """
        for k in batch:
            batch[k] = torch.stack((batch[k], batch[k], batch[k], batch[k],
                                    batch[k], batch[k], batch[k], batch[k],
                                    batch[k], batch[k], batch[k], batch[k],
                                    batch[k], batch[k], batch[k], batch[k]))
        """
        out = model(batch, gpu_temp_limit=gpu_temp_limit, plddt_stop=plddt_stop)  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        inference_time = time.perf_counter() - t
        # print("model(batch) time: ", inference_time)
...
def prep_output(out, batch, feature_dict, feature_processor, config_preset, multimer_ri_gap, subtract_plddt):
    ...
    # print("plddt_b_factors: ", plddt_b_factors)
