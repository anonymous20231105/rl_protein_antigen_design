...
from typing import Mapping, List, Any, Sequence, Tuple, Optional, Dict
import loss
import protein
...
from src.dir_openfold.tensor_utils import tensor_tree_map

from src.dir_openfold import residue_constants, data_pipeline, feature_pipeline, script_utils, trace_utils, tensor_utils

# WORK_DIR = "/home/gemhou/Study/hj_openfold_demo/srcs"
...
def openfold_name_2_seqtag(protein_name):
    tag_list = []
    seq_list = []
    fasta_dir = "../data/interim/fasta/fasta_" + protein_name  # WORK_DIR + "/dataset/fasta/fasta_" + protein_name
...
def add_data_args(parser: argparse.ArgumentParser):
    # dataset path
    parser.add_argument(
        '--uniref90_database_path', type=str, default="../data/raw/uniref90/uniref90.fasta",  #"/media/gemhou/新加卷/Dataset/uniref90/uniref90.fasta"
    )
    parser.add_argument(
        '--mgnify_database_path', type=str, default=None,  # "/media/gemhou/新加卷/Dataset/mgnify/mgy_clusters_2018_12.fa" "../data/raw/mgnify/mgy_clusters_2018_12.fa"
    )
    parser.add_argument(
        '--pdb70_database_path', type=str, default=None,  # "/media/gemhou/新加卷/Dataset/pdb70/pdb70"
    )
    parser.add_argument(
        '--uniclust30_database_path', type=str, default=None,  # "/media/gemhou/新加卷/Dataset/uniclust30/uniclust30_2018_08/uniclust30_2018_08"
    )
    parser.add_argument(
        '--bfd_database_path', type=str, default=None,  # "/media/gemhou/新加卷/Dataset/bfd/bfd_metaclust_clu_complete_id30_c90_final_seq.sorted_opt"
    )
    # other path
    parser.add_argument(
...
def generate_parser():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../data/processed",  # WORK_DIR + "/dataset",  # os.getcwd()
    ...
    parser.add_argument(
        ...
        type=str,
        default="../data/raw/finetuning_2.pt",  # WORK_DIR + "/../../data/finetuning_2.pt",  # None
...
def openfold_parser_2_args(parser):
...
def generate_args():
    ...
    args = openfold_parser_2_args(parser)
    return args
...
config_openfold = mlc.ConfigDict(
    {
        "data": {
            "common": {
                "feat": {
...
def model_config(
    ...
    c = copy.deepcopy(config_openfold)
    # TRAINING PRESETS
...
def _get_atom_positions(
    ...
    coords_with_mask = get_atom_coords(
...
def _process_single_hit(
    ...
    parsing_result = mmcif_parsing_parse(
...
def generate_alignment(args, local_alignment_dir, tag, tmp_fasta_path):
    logger.info(f"Generating alignments for {tag}...")
    print("Generating alignments for {tag}...", tag)
...
def precompute_alignments(tags, seqs, alignment_dir, args):
    ...
    for tag, seq in zip(tags, seqs):
        ...
        if (args.use_precomputed_alignments is None and not os.path.isdir(local_alignment_dir)) or tag == "TEMP":  #
            generate_alignment(args, local_alignment_dir, tag, tmp_fasta_path)
        else:
...
def calc_lddt_ca(pred_coords, all_atom_positions, all_atom_mask):
    pred_coords = pred_coords.to("cpu")
    lddt_ca_score = loss.lddt_ca(
        pred_coords,
        all_atom_positions,
        all_atom_mask,
    )
...
def generate_all_atom_positions(protein_name, chain_id="A"):
    file_id = protein_name
    path = "../data/interim/mmcif/mmcif_" + protein_name + "/" + protein_name + ".cif"  # WORK_DIR + "/dataset/mmcif/mmcif_" + protein_name + "/" + protein_name + ".cif"
    with open(path, "r") as f:
        mmcif_string = f.read()
    mmcif_object = mmcif_parsing_parse(file_id=file_id, mmcif_string=mmcif_string)
    mmcif_object = mmcif_object.mmcif_object
    all_atom_positions, all_atom_mask = get_atom_coords(
...
def calc_openfold_lddt(out, protein_name, chain_id_list):
    all_atom_mask_list = []
    all_atom_positions_list = []
    for chain_id in chain_id_list:
        all_atom_mask_temp, all_atom_positions_temp = generate_all_atom_positions(protein_name, chain_id=chain_id)
        all_atom_mask_list.append(all_atom_mask_temp)
        all_atom_positions_list.append(all_atom_positions_temp)
    # all_atom_mask_a, all_atom_positions_a = generate_all_atom_positions(protein_name, chain_id="A")
    # all_atom_mask_b, all_atom_positions_b = generate_all_atom_positions(protein_name, chain_id="B")
    # all_atom_mask_list = [all_atom_mask_a, all_atom_mask_b]
    # all_atom_positions_list = [all_atom_positions_a, all_atom_positions_b]
    all_atom_mask = torch.cat(all_atom_mask_list, dim=0)
    all_atom_positions = torch.cat(all_atom_positions_list, dim=0)

    pred_coords = out["final_atom_positions"]

    lddt_ca_score_mean = calc_lddt_ca(pred_coords, all_atom_positions, all_atom_mask)
    return lddt_ca_score_mean


def generate_pdb(args, feature_dict, feature_processor, output_protein_structure, processed_feature_dict, save_pdb_name="../data/interim/openfold_temp.pdb"):
    np_output_protein_structure = tensor_tree_map(lambda x: np.array(x.cpu()), output_protein_structure)
    np_processed_feature_dict = tensor_tree_map(lambda x: np.array(x[..., -1].cpu()), processed_feature_dict)
    unrelaxed_protein = script_utils.prep_output(
        np_output_protein_structure,
        np_processed_feature_dict,
        feature_dict,
        feature_processor,
        args.config_preset,
    ...
    unrelaxed_output_path = save_pdb_name
    with open(unrelaxed_output_path, "w") as fp:
        fp.write(protein.to_pdb(unrelaxed_protein))


def openfold_seqtag_2_structure(seq_list, tag_list, save_pdb_name=None, max_recycling_iters=None, gpu_temp_limit=None,
                                plddt_stop=False):
    if tag_list is None:
        tag = "TEMP"
        tag_list = [(tag, [tag])]
    ...
    # WORK_DIR + "/dataset/mmcif/mmcif_" + protein_name + "/"  # "./dataset/mmcif_" + protein_name + "/"
    # template_mmcif_dir = "../data/interim/mmcif/mmcif_" + protein_name + "/"
    template_mmcif_dir = "../data/interim/mmcif/mmcif_template/"
    ...
    if args.use_precomputed_alignments is None:
        alignment_dir = "../data/interim/alignments"  # os.path.join(output_dir_base, "alignments")
    else:
        alignment_dir = args.use_precomputed_alignments
    seq_sort_fn = lambda target: sum([len(s) for s in target[1]])
    sorted_targets = sorted(zip(tag_list, seq_list), key=seq_sort_fn)
    feature_dicts = {}
    model_generator = script_utils.load_models_from_command_line(
        config,
        args.model_device,
        ...
        for (tag, tags), seqs in sorted_targets:
            ...
            if feature_dict is None:
                ...
                if args.trace_model:
                    ...
                    feature_dict = trace_utils.pad_feature_dict_seq(
                        feature_dict,
                        rounded_seqlen,
                    )
            ...
            if args.trace_model:
                if rounded_seqlen > cur_tracing_interval:
                    trace_utils.trace_model_(model, processed_feature_dict)
            # print("Waiting for seconds...")
            output_protein_structure = \
                script_utils.run_model(model, processed_feature_dict, tag, args.output_dir,
                                       gpu_temp_limit=gpu_temp_limit, plddt_stop=plddt_stop)  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

            if save_pdb_name is not None:
                generate_pdb(args, feature_dict, feature_processor, output_protein_structure, processed_feature_dict, save_pdb_name)

            break
    return output_protein_structure


def calc_openfold_plddt(output_protein_structure):
    out_numpy = tensor_utils.tensor_tree_map(lambda x: np.array(x.cpu()), output_protein_structure)
    plddt = out_numpy["plddt"]
