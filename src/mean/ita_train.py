...
from src.mean.data import ITAWrapper, AAComplex
from src.mean.trainer.abs_trainer import TrainConfig
from src.mean.generate import set_cdr
from src.mean.evaluation.rmsd import kabsch
from src.mean.evaluation import pred_ddg
from src.mean.utils.logger import print_log
from src.mean.utils.random_seed import setup_seed


def prepare_efficient_mc_att(mode, data_path):
    from src.mean.trainer import MCAttTrainer
    from src.mean.data import EquiAACDataset
...
def parse():
    ...
    parser.add_argument('--update_freq', type=int, default=1,
                        help='Model update frequency. if not 1, true batch size will be freq * batch_size')
...
def main(args):
    ...
    print(f'mode: {mode}')
    print("args.pretrain_ckpt: ", args.pretrain_ckpt)
    # args.pretrain_ckpt = "summaries/ckpt/mean_CDR3_111/version_1/checkpoint/"
    model = torch.load(args.pretrain_ckpt + "epoch19_step3500.ckpt", map_location='cpu')
    device = torch.device('cpu' if args.gpu == -1 else f'cuda:{args.gpu}')
    model.to(device)
    dataset, Trainer = prepare_efficient_mc_att(mode, args.test_set)
